...
public class MoveAdapter extends RecyclerView.Adapter<MoveAdapter.ViewHolder> {
    ...
    public MoveAdapter(List<Query.DataBean.Info> data, Context context) {
        this.data = data;
    ...
    public MoveAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_BUTTON) {
            return new MoveAdapter.ViewHolder(inflater.inflate(R.layout.list_button, parent, false));
        } else {
            return new MoveAdapter.ViewHolder(inflater.inflate(R.layout.list_information, parent, false));
        }
    }

    @Override
    public void onBindViewHolder(MoveAdapter.ViewHolder holder, final int position) {
